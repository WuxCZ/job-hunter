...
def cmd_run(
    ...
    *,
    ...
    print(
        f"### JOBHUNTER_RUN_BEGIN ts={datetime.now().isoformat(timespec='seconds')} "
        f"listings={len(listings)} dry_run={dry_run} ###",
        flush=True,
    )
    run_outcome = "done"
    try:
        ...
        run_outcome = f"{exc.__class__.__name__}"
        print(
            f"Hotovo (přerušeno: {exc.__class__.__name__}) | applied={applied_count} "
            f"skipped={skipped_count} failed={failed_count} dry_run={dry_run}",
            flush=True,
        )
        raise
    finally:
        print(
            f"### JOBHUNTER_RUN_END outcome={run_outcome} applied={applied_count} "
            f"skipped={skipped_count} failed={failed_count} dry_run={dry_run} ###",
            flush=True,
        )
